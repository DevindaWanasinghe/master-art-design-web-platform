...
const authenticateToken = require('../main/authmiddleware');
...
router.post('/register',async(req ,res)=>{
    ...
    isMailinDb().then((data)=>{
        if(data.length>0) return res.status(400).json({ message: 'User already exists' });
            ...
            db.query('INSERT INTO user (userName, phone, email, password) VALUES (?, ?, ?, ?)', 
            [userName, phone, email, hashedPassword], (err, result) => {
                if (err) {
...
router.get('/verify/:token',(req, res) => {
    ...
    jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
        ...
        db.query('UPDATE user SET verified = TRUE WHERE email = ?',[decoded.email],(err,result) => {
...
router.post('/login',async(req,res)=>{
    const { email, password } = req.body;
        ...
        const token = jwt.sign({ id: result[0].userId }, process.env.JWT_SECRET, { expiresIn: '1h' });
...
router.get('/Account', authenticateToken, (req, res) => {
  db.query('SELECT userName, email, phone ,imageUrl FROM user WHERE userId = ?', [req.user.id], (err, result) => {
    if (err) {
